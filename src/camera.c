...
int virtual_shot(camera *camera,double f,double v,double h,double o,double vv,double vh){
    ...
    asprintf(&msj,"%s:     focal: %6.2f mm",camera->name,f);verbose(L_ACCT,msj);free(msj);
    asprintf(&msj,"%s: υ: sensor: %6.2f • %6.2f mm",camera->name,h,v);verbose(L_ACCT,msj);free(msj);
    asprintf(&msj,"%s: υ:      α: %6.2f • %6.2f °",camera->name,a_v,a_h);verbose(L_ACCT,msj);free(msj);
    a_v=a_v*(1-(o/2));a_h=a_h*(1-(o/2));
    asprintf(&msj,"%s: υ:     α': %6.2f • %6.2f °",camera->name,a_v,a_h);verbose(L_ACCT,msj);free(msj);
    asprintf(&msj,"%s: ν: sensor: %6.2f • %6.2f mm",camera->name,vh,vv);verbose(L_ACCT,msj);free(msj);
    asprintf(&msj,"%s: ν:      α: %6.2f • %6.2f °",camera->name,va_v,va_h);verbose(L_ACCT,msj);free(msj);
