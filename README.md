...
<p align="justify">
In this file there are also some #defines to control the motor's speed and movement, there are three sets; one for default mode(<code>_DEF</code>), another used with the argument `-c` for Fast Mode(`_FAST`) and the last, used with the argument `-z` for Slow mode(`_SLOW`).
</p>
